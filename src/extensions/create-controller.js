module.exports = toolbox => {
  ...
  async function resolver_destination_package(config_file, package) {
    ...
    if (package == null) {
      resolver_package =
        resolver_package + '.' + config_file.javaProjects.packageController
    }
  ...
  async function createController(class_name_param, package) {
    ...
    let src_main_destination = `${(await destination).full_path_main}/${
      props.class_name
    }.java`
    ...
    print.success(
      `   Generated ${(await destination).full_path_main}/${props.class_name}`
    )

    await template.generate({
      template: 'controller-test-blank.js.ejs',
      target: `${(await destination).full_path_test}/${
        props.class_name_test
      }.java`,
      props: props
    })
    print.info('invoke test_unit')
    print.success(
      `   Generated ${(await destination).full_path_test}/${
        props.class_name_test
      }`
    )
  }
